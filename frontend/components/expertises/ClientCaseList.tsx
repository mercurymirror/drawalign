...
export function ClientCaseList({ items, onSelect }: Props) {
	return (
		<Section noPadding className="bg-accent-peach pt-0 pb-0 md:pt-0 md:pb-0">
			<div className="grid grid-cols-1 gap-5 md:grid-cols-2">
				{items.map((item, index) => (
					<Fragment key={item.id}>
						{/* Colored card */}
						<div
							className={cn("flex flex-col gap-8 rounded-2xl p-8 lg:p-16", bgMap[item.background])}
						>
							<h3 className="text-3xl text-white tracking-tighter lg:text-[40px]">
								{item.card_title ?? item.title}
							</h3>
							{item.features && item.features.length > 0 && (
								<ul className="flex flex-col gap-4">
									{item.features.map((f) => (
										<li key={f.id} className="border-white border-l pl-4 text-sm text-white">
